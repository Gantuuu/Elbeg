...
// .wrangler/tmp/bundle-wNWK5P/checked-fetch.js
...
__export(schema_exports, {
  ...
  generatedMealKitComponents: () => generatedMealKitComponents,
  generatedMealKitComponentsRelations: () => generatedMealKitComponentsRelations,
  generatedMealKits: () => generatedMealKits,
  generatedMealKitsRelations: () => generatedMealKitsRelations,
  ...
  insertGeneratedMealKitComponentSchema: () => insertGeneratedMealKitComponentSchema,
  insertGeneratedMealKitSchema: () => insertGeneratedMealKitSchema,
  insertMealKitComponentSchema: () => insertMealKitComponentSchema,
  insertMealKitSchema: () => insertMealKitSchema,
  insertMediaSchema: () => insertMediaSchema,
  insertNavigationItemSchema: () => insertNavigationItemSchema,
  insertNonDeliveryDaySchema: () => insertNonDeliveryDaySchema,
  ...
  mealKitComponents: () => mealKitComponents,
  mealKitComponentsRelations: () => mealKitComponentsRelations,
  mealKits: () => mealKits,
  mealKitsRelations: () => mealKitsRelations,
    ...
    for (const productId of productIds) {
      ...
      if (product) {
        ...
        componentData.push({
          ...
          // type mismatch handled in insert? converted to number in db? schema says real.
          price: price.toString()
          // Schema says real (number in sqlite), but TS might expect string if drizzle-zod inferred from decimal.
...
app2.post("/", requireAdmin, async (c) => {
  ...
  try {
    ...
    const productData = {
      ...
      price: parseFloat(parsedData.price || body["price"]),
      ...
      minOrderQuantity: parseFloat(parsedData.minOrderQuantity || body["minOrderQuantity"] || "1"),
      storeId: parsedData.storeId ? parseInt(parsedData.storeId) : body["storeId"] ? parseInt(body["storeId"]) : void 0
    };
...
app2.put("/:id", requireAdmin, async (c) => {
  ...
  if (parsedData.minOrderQuantity || body["minOrderQuantity"]) {
    updateData.minOrderQuantity = parseFloat(parsedData.minOrderQuantity || body["minOrderQuantity"]);
  }
  if (parsedData.storeId || body["storeId"]) {
    updateData.storeId = parseInt(parsedData.storeId || body["storeId"]);
  }
  if (file && file instanceof File) {
    const fileName = `products/${Date.now()}_${file.name}`;
    await c.env.BUCKET.put(fileName, file);
...
// .wrangler/tmp/bundle-wNWK5P/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-wNWK5P/middleware-loader.entry.ts
